...
//!
//! Hand-written rather than generated by `wasm-bindgen`, for one reason: the
//! module's size is budgeted, and a whole browser client has to fit inside
//! it. A binding generator brings a JS shim, a descriptor section and a lot
//! of glue for a surface that is eight functions wide and passes nothing but
//! integers and byte ranges. `web/build.sh` measures what actually ships
//! against the budget, so this is a decision with a number under it rather
//! than a taste.
//!
//! ## Two ways in
//!
...
//! one imported synchronous function. That import is where the synchronous
//! read path lands: JS downloads the object into the Origin Private File
//! System once, asynchronously, and then answers `supdb_host_read` from a
...
//! download budget this library is designed around is 32 MB, so the limit is
//! two orders of magnitude clear of anything a browser will hand it, and
//! keeping it out of BigInt keeps the JS glue plain. An object at or over the
//! limit is refused at open rather than wrapped.
...
impl Bytes for HostBytes {
    ...
    // No `advise_random` either: a no-op where there is no mapping.
}
...
enum AnyBlob {
    ...
    /// dictionary by range. Point reads are not on it, on purpose.
...
/// The data ranges a read of these keys will touch.
///
...
/// Values under a key, without decoding them.
///
...
/// A posting list of four-byte ordinals is the case this exists for: a
/// caller wanting per-key totals without reading the values.
///
/// # Safety
/// `kptr` must point at `klen` readable bytes.
#[no_mangle]
...
/// Every value of a key, framed into the out buffer.
///
...
/// Keys in order from `from`, each with its value count.
///
...
// ------------------------------------------------------- sparse dictionary --
//
// The key index fetched by range, for a dictionary too large to fetch
...
/// so every later lookup plans its records with no dependent read.
#[no_mangle]
