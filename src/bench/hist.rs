...
impl Hist {
    ...
    /// Percentiles as milliseconds, which is the unit the tail is read in.
    pub fn to_json(&self) -> J {
        let ms = |ns: u64| J::fp(ns as f64 / 1e6, 5);
        let mut pcts = vec![
            ("count".to_string(), J::u(self.total)),
            ("mean_ms".to_string(), J::fp(self.mean() / 1e6, 5)),
            ("min_ms".to_string(), ms(self.min())),
        ];
        for p in REPORTED {
            let key = if p.fract() == 0.0 {
                format!("p{}_ms", *p as u64)
            } else {
                format!("p{}_ms", p.to_string().replace('.', "_"))
            };
            pcts.push((key, ms(self.percentile(*p))));
        }
        pcts.push(("max_ms".to_string(), ms(self.max())));
        // The ratio that says whether the mean was ever an honest summary.
        let tail = if self.mean() > 0.0 {
            self.percentile(99.9) as f64 / self.mean()
        } else {
            0.0
        };
        pcts.push(("p99_9_over_mean".to_string(), J::fp(tail, 2)));
        J::O(pcts)
    }

    /// A compact CDF for plotting, as (percentile, milliseconds) pairs.
    pub fn cdf_json(&self) -> J {
        let points = [
            0.0, 10.0, 25.0, 50.0, 75.0, 90.0, 95.0, 99.0, 99.5, 99.9, 99.99, 99.999, 100.0,
        ];
        J::arr(
            points
                .iter()
                .map(|p| {
                    jobj! {
                        "p" => J::fp(*p, 3),
                        "ms" => J::fp(self.percentile(*p) as f64 / 1e6, 5),
                    }
                })
                .collect(),
        )
